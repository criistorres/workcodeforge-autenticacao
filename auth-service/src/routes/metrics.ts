...
const basicAuth = (_req: Request, res: Response, next: Function) => {
  ...
  // Simple hardcoded credentials for metrics (should be in env in production)
...
router.get('/', basicAuth, async (_req: Request, res: Response) => {
  try {
    ...
    // Update user metrics
    metricsCollector.updateUserMetrics(totalUsers, activeUsers, registeredToday);
...
router.get('/summary', basicAuth, async (_req: Request, res: Response) => {
  try {
...
router.get('/health', async (_req: Request, res: Response) => {
  try {
    ...
    // Check response time
    if (metrics.requests.averageResponseTime > 5000) { // More than 5 seconds
      status = 'degraded';
...
router.post('/reset', basicAuth, (_req: Request, res: Response) => {
  try {
